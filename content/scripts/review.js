// Review dialog logic for arxiv_marker_ccf2026. Runs in the modal dialog window's scope.
...
function zmReviewLoad() {
  ...
  ZM_STR = {
    ...
    cols: zh
      ? ["", "标题", "会议/期刊", "CCF / CORE", "类型变更", "来源", "置信", "引用", "将写入字段"]
      : ["", "Title", "Venue", "CCF / CORE", "Type change", "Sources", "Conf.", "Cites", "Fields to write"],
  ...
  for (const res of resolutions) {
    ...
    const tiers = [
      res.ccf_tier ? `CCF ${res.ccf_tier}` : "",
      res.core_tier ? `CORE ${res.core_tier}` : "",
    ].filter(Boolean).join(" · ") || (res.kind === "journal" ? "journal" : "—");
    tr.appendChild(zmTd(tiers, "white-space: nowrap;"));
    tr.appendChild(
      zmTd(eligible ? `${res.current_item_type} → ${res.target_item_type}` : ZM_STR.nochange, "white-space: nowrap;")
    );
    const sourceText = (res.sources || []).join(", ") || "—";
    const sourceCell = zmTd(sourceText, "max-width: 170px; word-break: break-word;");
    if (res.evidence && res.evidence.length) sourceCell.title = res.evidence.join("\n");
    if (res.diagnostics) {
      const d = res.diagnostics;
      sourceCell.textContent += `\n${d.status} · ${((d.elapsed_ms || 0) / 1000).toFixed(1)}s`;
      sourceCell.title += "\n" + (d.sources || []).map((v) => `${v.source}: ${v.status} ${v.elapsed_ms || 0}ms${v.reason ? " " + v.reason : ""}`).join("\n");
    }
